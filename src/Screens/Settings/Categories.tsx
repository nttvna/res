...
import { useLazyGetCategoryQuery, useLazyRemoveCategoryQuery, useUpdateCategoryTypeMutation } from '../../Apis/settings';
import FlatListEmpty from '../../Components/FlatListEmpty';
import IconButton from '../../Components/IconButton';
import ImageWithFallback from '../../Components/ImageWithFallback';
import LoadingComponent from '../../Components/Loading';
import Layout from '../../Layout';
import { CategoryReportItem, CategoryTypeParam } from '../../Models/setting';
...
const Categories = () => {
    ...
    const [getData, { data, isLoading }] = useLazyGetCategoryQuery();
    const [deleteItem] = useLazyRemoveCategoryQuery();
...
console.log(data);
    ...
    const EditItem = (item: CategoryReportItem) => {
        /*  setItemParam({
             Id: item.Id,
             Name: item.Name,
    ...
    const renderItem = (item: CategoryReportItem, index: number) => {
        return <View style={{
            ...
            <View style={{ flex: 4 / 10, flexDirection: 'row', alignItems: 'center' }}>
                <View style={{ width: 90, height: 90, paddingRight: 20, alignItems: 'center' }}>
                    <ImageWithFallback path={item.ImageUrl} aspectRatio={4 / 3} />
                </View>
                <Text style={[s.itemcenter, { alignItems: 'flex-start', textTransform: 'capitalize', fontWeight: 'bold', marginLeft: 14 }]}>{item.Name}</Text>
            </View>
            <View style={{ flex: 2 / 10 }}>
                <Text style={s.itemcenter}>{item.SortIndex == 0 ? '' : item.SortIndex}</Text>
            </View>
    ...
    const deleteCategory = (itm: CategoryReportItem) => {
    ...
    const DeleteConfirm = (itm: CategoryReportItem) => {
        Alert.alert(
            'Confirm',
